...
local function GetEntityType(entity)
    if entity.Parent == workspace.Characters then
        return "Player"
    elseif entity.Parent == workspace.Titans then
        return "Titan"
    elseif entity.Parent == workspace.Unclimbable.Reloads then
        return "Refill"
    else
        return nil
    end
...
local function GetEntityName(entity, entityType)
    if entityType == "Player" then
        return entity.Name
    elseif entityType == "Titan" then
        return entity.Name .. " (Titan)"
    elseif entityType == "Refill" then
        return entity.Name
    else
        return entity.Name
    end
...
local function ApplyESPToEntity(entity)
    ...
    if (entityType == "Player" or entityType == "Titan") and config.Healthbar then
    ...
    Connection = RunService.RenderStepped:Connect(function()
        ...
        if entityType == "Titan" and not SimpleESP.ShowTitans then
            HideAllComponents()
            return
        end

        if entityType == "Refill" and not SimpleESP.ShowRefills then
            HideAllComponents()
            return
        end
        ...
        if entity and entity:FindFirstChild("HumanoidRootPart") then
            ...
            if (entityType == "Player" or entityType == "Titan") and not Humanoid then
                HideAllComponents()
                return
            end
            ...
            if OnScreen and Dist <= SimpleESP.MaxDistance then
                ...
                -- For refills, use smaller size
                if entityType == "Refill" then
                    w = 1.5 * scaleFactor
                    h = 1.5 * scaleFactor
                end
                ...
                -- Draw box for Players and Titans
                if (entityType == "Player" or entityType == "Titan") and config.Box then
                ...
                if config.Distance and config.Distance.Enabled and (entityType == "Player" or entityType == "Titan") then
                ...
                -- Draw healthbar for Players and Titans
                if (entityType == "Player" or entityType == "Titan") and Humanoid and config.Healthbar and config.Healthbar.Enabled then
...
local function InitializeESP()
    ...
    -- Initialize Titans ESP
    if workspace:FindFirstChild("Titans") then
        for _, titan in pairs(workspace.Titans:GetChildren()) do
            coroutine.wrap(ApplyESPToEntity)(titan)
        end

        workspace.Titans.ChildAdded:Connect(function(titan)
            wait(0.5)
            coroutine.wrap(ApplyESPToEntity)(titan)
        end)
    end

    -- Initialize Refill Points ESP
    if workspace:FindFirstChild("Unclimbable") and workspace.Unclimbable:FindFirstChild("Reloads") then
        for _, refill in pairs(workspace.Unclimbable.Reloads:GetChildren()) do
            coroutine.wrap(ApplyESPToEntity)(refill)
        end

        workspace.Unclimbable.Reloads.ChildAdded:Connect(function(refill)
            wait(0.5)
            coroutine.wrap(ApplyESPToEntity)(refill)
        end)
    end
