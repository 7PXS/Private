--[[
    KiwiSense ESP Library v2.4 - Fixed
    Fixes:
        - Mob ESP Text drift/offset (Removed redundant frame-by-frame reparenting).
        - Posture Bar filling logic (Dynamic sizing relative to box dimensions).
        - Posture Bar default value handling (Prevents crash/empty bar).
]]
...
local function LoadLibrary()
    ...
    local MiscOptions = {
        ...
        ["PostureBar"] = false;
        ["PostureBar_Position"] = "Right";
        ["PostureBar_Width"] = 3;
        ["PostureBar_Color"] = { Color = Color3.fromRGB(255, 207, 125), Transparency = 0 };
    ...
    do
        ...
        do 
            ...
            function Esp.CreateObject( player, typechar )
                ...
                local Items = Data.Items; do
                    ...
                    Items.PostureBarAccent = Esp:Create("Frame", {
                        ...
                        Size = dim2(1, -2, 0, -2),
                        BorderSizePixel = 0,
                        BackgroundColor3 = rgb(255, 207, 125) 
                    })

                    local PostureGradient = Esp:Create("UIGradient", {
                        Parent = Items.PostureBarAccent,
                        Rotation = 90,
                        Color = rgbseq{rgbkey(0, rgb(255, 207, 125)), rgbkey(1, rgb(255, 207, 125))}
                    })
                    
                    local PostureStroke = Instance.new("UIStroke")
                    PostureStroke.Color = Color3.new(0,0,0)
                    PostureStroke.Thickness = 1
                    PostureStroke.Parent = Items.PostureBar
                ...
                Data.RefreshDescendants = function() 
                    ...
                    if not rank and player:IsA("Player") then
                        local leaderstats = player:FindFirstChild("leaderstats")
                        if leaderstats then
                            local rankValue = leaderstats:FindFirstChild("Rank")
                            if rankValue then rank = tostring(rankValue.Value) end
                        end
                    end
                    Data.Info.Rank = rank

                    pcall(function()
                        local entityFolder = Workspace:FindFirstChild("World")
                        if entityFolder then
                            entityFolder = entityFolder:FindFirstChild("Entities")
                            if entityFolder then
                                entityFolder = entityFolder:FindFirstChild(player.Name)
                                if entityFolder then
                                    local infoFolder = entityFolder:FindFirstChild("Info")
                                    if infoFolder then
                                        local posture = infoFolder:FindFirstChild("Posture")
                                        if posture and (posture:IsA("IntValue") or posture:IsA("NumberValue")) then
                                            Data.Info.Posture = posture.Value or 0
                                            Data.Info.MaxPosture = posture:GetAttribute("MaxValue") or 100
                                            
                                            if not posture:GetAttribute("KiwiPostureConnected") then
                                                posture:SetAttribute("KiwiPostureConnected", true)
                                                posture.Changed:Connect(function(newVal)
                                                    Data.Info.Posture = newVal
                                                end)
                                            end
                                        end
                                    end
            ...
            function Esp.Update() 
                ...
                for _,Data in pairs(Esp.Players) do
                    ...
                    -- POSTURE BAR FIXED LOGIC
                    if MiscOptions.PostureBar and Items.PostureBar then
                        local pos = MiscOptions.PostureBar_Position
                        local targetParent = Items[pos] or Items.Right
                        
                        Items.PostureBar.Parent = targetParent
                        Items.PostureBar.Name = pos
                        
                        local thickness = MiscOptions.PostureBar_Width
                        local isHorizontal = (pos == "Top" or pos == "Bottom")

                        -- FIX: Dynamic Sizing based on Box Dimensions
                        if isHorizontal then
                            Items.PostureBar.Size = dim2(1, 0, 0, thickness) -- Full Width, Fixed Height
                        else
                            Items.PostureBar.Size = dim2(0, thickness, 1, 0) -- Fixed Width, Full Height
                        end
                        
                        local maxPost = Data.Info.MaxPosture or 100
                        if maxPost == 0 then maxPost = 1 end
                        local posturePercent = math.clamp((Data.Info.Posture or 0) / maxPost, 0, 1)
                        
                        if isHorizontal then
                            Items.PostureBarAccent.Size = dim2(posturePercent, -2, 1, -2)
                            Items.PostureBarAccent.Position = dim2(0, 1, 0, 1)
                            local grad = Items.PostureBarAccent:FindFirstChildOfClass("UIGradient")
                        ...
                        Items.PostureBarAccent.BackgroundColor3 = MiscOptions.PostureBar_Color.Color
                    else
                        if Items.PostureBar then Items.PostureBar.Parent = Esp.Cache end
                    end

                    local displayName = tostring(player.Name)
                    if MiscOptions.Rank_Text and Data.Info.Rank then
                        displayName = displayName .. " [" .. Data.Info.Rank .. "]"
                    end
                    
                    if Items.Text.Text ~= displayName then Items.Text.Text = displayName end 
            ...
            function Esp.CreateMobESP(mobEntity)
                ...
                for _, ItemParentor in pairs({Items.Left, Items.Top, Items.Bottom}) do
                    Esp:Connection(ItemParentor.ChildAdded, function()
                        task.wait(0.1)
                        if ItemParentor.Parent == nil then return end
                        ItemParentor.Parent = Items.Holder
                ...
                for _, TextContainer in pairs({Items.TopTexts, Items.BottomTexts}) do
                    Esp:Connection(TextContainer.ChildAdded, function()
                        task.wait(0.1)
                        if TextContainer.Parent == nil then return end
                        if TextContainer.Parent.Parent ~= Items.Holder then
                            TextContainer.Parent.Parent = Items.Holder
                        end
                    end)
                    
                    Esp:Connection(TextContainer.ChildRemoved, function()
                        task.wait(0.1)
                        if #TextContainer:GetChildren() == 0 then
                            if TextContainer.Parent == nil then return end 
                            TextContainer.Parent.Parent = Esp.Cache
                        end 
                    end)
                end
            ...
            function Esp.UpdateMobESP()
                ...
                for _, Data in pairs(Esp.MobESPs) do
                    ...
                    if MobOptions.Healthbar then
                        local targetParent = MobOptions.Healthbar_Position == "Left" and Items.Left or Items.Left
                        Items.Healthbar.Parent = targetParent
                        Items.Healthbar.Name = MobOptions.Healthbar_Position
                    ...
                    -- MOB TEXT POSITION FIX: Only reparent if necessary (matches Player ESP logic)
                    if MobOptions.Name_Text then
                        Items.Text.Text = Data.Info.Name
                        Items.Text.TextSize = MobOptions.Name_Text_Size
                        if Items.Text.Parent ~= Items.TopTexts then
                            Items.Text.Parent = Items.TopTexts
                        end
                    else
                        if Items.Text.Parent ~= Esp.Cache then
                    ...
                    if MobOptions.Distance_Text then
                        ...
                        if Items.Distance.Parent ~= Items.BottomTexts then
                            Items.Distance.Parent = Items.BottomTexts
                        end
                    else
                        if Items.Distance.Parent ~= Esp.Cache then
            ...
            function Esp.RefreshElements(key, value)
                for _,Data in pairs(Esp.Players) do
                    ...
                    if key == "PostureBar" then
                        Items.PostureBar.Parent = value and Items[Items.PostureBar.Name] or Esp.Cache
                    end
                    
                    if key == "PostureBar_Position" then
                             local isEnabled = MiscOptions.PostureBar
                            Items.PostureBar.Parent = isEnabled and Items[value] or Esp.Cache
                            Items.PostureBar.Name = value
                    end
        ...
        Esp.ScanForMobs()
        local entitiesFolder = Workspace:FindFirstChild("World")
        if entitiesFolder then entitiesFolder = entitiesFolder:FindFirstChild("Entities") end
        if entitiesFolder then
        ...
        Esp.Loop = RunService:BindToRenderStep("EspLoop", Enum.RenderPriority.Camera.Value + 1, function()
            Esp.Update()
            Esp.UpdateMobESP()
        end)
